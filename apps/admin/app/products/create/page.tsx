...
export default function ProductCreatePage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const [cats, leads] = await Promise.all([getAdminCategoryItems(), getAdminRetailers()])
        setCategories(cats)
        setRetailers(leads)
        setForm((prev) => ({ ...prev, categoryId: cats[0]?.id || "" }))
      } catch (err) {
        setError(err instanceof Error ? err.message : "Unable to load create form options.")
      }
    })()
  }, [])
  ...
  return (
    <div>
      ...
      <h1 style={{ marginBottom: 6 }}>Create Product</h1>
      ...
      <form onSubmit={submit} style={{ display: "grid", gap: 10, maxWidth: 760 }}>
        ...
        <input placeholder="Image URL (required in full migration)" value={form.imageUrl} onChange={(e) => setForm((p) => ({ ...p, imageUrl: e.target.value }))} />
